...
const Navbar = () => {
  ...
  // Helper function to get proper profile image
  const getProfileImage = () => {
  // 🧩 No user data yet
  if (!user) return assets.profile;

  let image = user.profileImage;

  // 🧩 If no image found, use default avatar
  if (!image) return assets.profile;

  // 🧩 If image already in correct Data URL format (includes "data:")
  if (image.startsWith("data:")) {
    return image;
  }

  // 🧩 Otherwise, assume raw base64 string and add proper prefix
  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
      ...
      <div className="hidden sm:flex items-center gap-8">
        ...
        {user && <NavLink to="/listings">Available to Rent</NavLink>}
      ...
      {open && (
        <div className="absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex flex-col gap-4 px-6 text-base md:hidden z-50">
          ...
          {user && (
            <NavLink to="/listings" onClick={() => setOpen(false)} className="hover:text-primary">
              Available to Rent
            </NavLink>
          )}
