    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {user && (
                    <>
                        <NavLink to='/listings'>Lends</NavLink>
                    </>
                )} 

                <div className="hidden lg:flex items-center text-sm gap-2 border border-gray-300 px-3 rounded-full">
                    <input onChange={(e)=>setSearchQuery(e.target.value)} className="py-1.5 
                    w-full bg-transparent outline-none placeholder-gray-500" alt="search" type="text" 
                ...
                (
                    <div className="relative group">
                        <img src={assets.profile} className="w-10" alt="" />
                        ...
                        border-gray-200 py-2.5 w-30 rounded-md text-sm z-40">
                            ...
                            <li onClick={()=> navigate("/rentals")} className="p-1.5 pl-3 hover:bg-primary/10 cursor-pointer">Rentals</li>
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                ...
                {user && (
                <>
                <NavLink to='/listings' onClick={() => setOpen(false)}>Lends</NavLink>
                </>
                )}
                {!user ? (
                <button onClick={() => {
